...
function normal(value) {
  return text(value, 1200).toLowerCase().replace(/[\s，。、！？,.!?：:；;()（）]+/g, '');
}
...
export function normaliseSchema(schema) {
  ...
  const properties = {};
  Object.keys(schema.properties).slice(0, 20).forEach((name) => {
    if (!/^[a-zA-Z][a-zA-Z0-9_-]{0,39}$/.test(name)) return;
    const raw = schema.properties[name] || {};
    const type = /^(string|number|integer|boolean)$/.test(raw.type) ? raw.type : 'string';
    const property = {
      type,
      title: text(raw.title || name, 80),
      description: text(raw.description, 160),
      contextKey: text(raw.contextKey, 60),
      format: /^(email|url|phone|contact)$/.test(raw.format) ? raw.format : '',
      prefixes: Array.isArray(raw.prefixes)
        ? raw.prefixes.slice(0, 8).map(item => text(item, 30)).filter(Boolean)
        : []
    };
    if (Array.isArray(raw.enum)) {
      property.enum = raw.enum.slice(0, 20).map(item => text(item, 80)).filter(Boolean);
    }
...
export function normaliseTool(tool) {
  ...
  return {
    ...
    keywords: Array.isArray(tool.keywords)
      ? tool.keywords.slice(0, 30).map(item => text(item, 60).toLowerCase()).filter(Boolean)
      : [],
    examples: Array.isArray(tool.examples)
      ? tool.examples.slice(0, 20).map(item => text(item, 160)).filter(Boolean)
      : [],
    excludeKeywords: Array.isArray(tool.excludeKeywords)
      ? tool.excludeKeywords.slice(0, 20).map(item => text(item, 60).toLowerCase()).filter(Boolean)
      : [],
...
export function scoreTool(tool, query) {
  ...
  if (!q || tool.excludeKeywords.some(item => item && q.includes(normal(item)))) {
  ...
  tool.keywords.forEach((keyword) => {
    ...
    if (!key) return;
    const current = q.includes(key) ? Math.min(0.92, 0.62 + key.length * 0.035) : similarity(q, key) * 0.62;
    if (current > score) {
      score = current;
  ...
  if (descriptionSimilarity >= 0.34 && 0.1 + descriptionSimilarity * 0.52 > score) {
...
function findPrefixed(query, prefixes) {
  for (let i = 0; i < prefixes.length; i++) {
    const re = new RegExp(escapeRegExp(prefixes[i]) + '\\s*(?:是|為|=|:|：)?\\s*([^，。！？,!?]{1,120})', 'i');
...
function valueForProperty(name, property, query, context, allowWhole) {
  ...
  const value = findPrefixed(query, property.prefixes.concat([property.title]).filter(Boolean));
  
  if (property.enum) {
    const selected = property.enum.find(item => normal(query).includes(normal(item)));
...
export function validate(schema, input) {
  ...
  Object.keys(schema.properties).forEach((name) => {
    ...
    if (property.type === 'integer' || property.type === 'number') {
      ...
      if (property.format === 'contact' && !(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) || /^(?:\+?\d[\s().-]*){8,18}$/.test(value))) {
...
export function extract(tool, query, context, existing, onlyNames, allowWhole) {
  ...
  const names = Array.isArray(onlyNames) && onlyNames.length ? onlyNames : Object.keys(properties);
  names.forEach((name) => {
    if (!properties[name] || args[name] != null) return;
    const value = valueForProperty(name, properties[name], String(query || ''), context || {}, !!allowWhole && names.length === 1);
  ...
  return {
    ...
    missing: tool.inputSchema.required.filter(name => validation.args[name] == null || validation.args[name] === ''),
...
export function initToolsEngine(setting = {}, aiAvatarWidget) {
  ...
  function parameterPrompt(tool, name, errorText) {
    ...
    const choices = property.enum && property.enum.length
  ...
  function offerHostTool(tool, query, routeMeta, args) {
    const callId = `tool-${Date.now()}-${++aiAvatarWidget.brainEngine.chatSeq}`;
