  ...
  describe('executeToolCallsLoop', () => {
    ...
    beforeEach(() => {
      mockBrainEngine = {
        ...
        executeTool: vi.fn(
          async (_tool: ToolDefinition, args: Record<string, unknown>) => ({
            ok: true,
            data: args
          })
        ),
        offerToolConfirmation: vi.fn(),
        onToolNotFound: vi.fn(async ({ toolName }: { toolName: string }) => ({
          ok: false,
          error: `Custom not found: ${toolName}`
        })),
        onToolError: vi.fn(async ({ error }: { error: Error }) => ({
          ok: false,
    ...
    it('should safely return if toolCalls is empty', async () => {
      ...
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
    ...
    it('should handle tool not found and trigger onToolNotFound hook', async () => {
      const toolCallResponse = {
        toolCalls: [
          {
            id: 'call_1',
            function: { name: 'missing_tool', arguments: '{"q": 1}' }
          }
        ],
        message: { content: '' }
      };
      ...
      expect(
        mockBrainEngine.onToolNotFound as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
      expect(
        (mockBrainEngine.aiProvider as { chat: ReturnType<typeof vi.fn> }).chat
      ).toHaveBeenCalled();
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('AI Provider Summary of Tool');
    });

    it('should handle tool execution error and trigger onToolError hook', async () => {
      mockBrainEngine.executeTool = vi
        .fn()
      ...
      const toolCallResponse = {
        toolCalls: [
          {
            id: 'call_1',
            function: { name: 'registered_tool', arguments: '{"q": 1}' }
          }
        ],
        message: { content: '' }
      };
      ...
      expect(
        mockBrainEngine.onToolError as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
    ...
    it('should fallback to string or message in lastResult when summary response is empty', async () => {
      ...
      const toolCallResponse1 = {
        toolCalls: [
          {
            id: 'call_str',
            function: { name: 'registered_tool', arguments: '{}' }
          }
        ],
        message: { content: '' }
      };
      ...
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('直接字串結果');

      // 2. lastResult as object with message property
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue({ message: '物件訊息回傳' });
      ...
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('物件訊息回傳');
    });

    it('should handle tool confirmation required and resume after confirmation', async () => {
      let confirmationContext:
        | { onConfirmResume: (res: Record<string, unknown>) => Promise<void> }
        | undefined;
      mockBrainEngine.offerToolConfirmation = vi.fn(
        (
          _tool: unknown,
          _args: unknown,
          ctx: {
      ...
      expect(
        mockBrainEngine.offerToolConfirmation as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
    ...
    it('should execute WebLLM streaming second round summary', async () => {
      ...
      expect(
        mockBrainEngine.onStreamStart as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
      expect(
        (mockBrainEngine.llm as { chat: ReturnType<typeof vi.fn> }).chat
      ).toHaveBeenCalled();
      expect(
        mockBrainEngine.onStreamChunk as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('Chunk 1');
      expect(
        (mockBrainEngine.memory as { addTurn: ReturnType<typeof vi.fn> })
          .addTurn
      ).toHaveBeenCalledWith('assistant', 'WebLLM Tool Summary');
      expect(
        mockBrainEngine.onStreamEnd as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('WebLLM Tool Summary');
      expect(
        mockBrainEngine.triggerRollingSummaryIfNeeded as ReturnType<
          typeof vi.fn
        >
      ).toHaveBeenCalled();
    });
    ...
    it('should fallback to lastResult error, message, or default string when summary response is empty', async () => {
      ...
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue({ ok: false, error: 'Database timeout' });
      ...
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('Database timeout');

      // 2. AI Provider with empty summary and lastResult is a plain string
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue('Plain result string');
      await executeToolCallsLoop(
        mockBrainEngine as unknown as BrainEngine,
        toolCallResponse,
        [],
      ...
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('Plain result string');

      // 3. AI Provider with empty summary and lastResult has message
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue({ ok: true, message: 'Updated 5 items' });
      await executeToolCallsLoop(
        mockBrainEngine as unknown as BrainEngine,
        toolCallResponse,
        [],
      ...
      expect(
        mockBrainEngine.emitAnswer as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('Updated 5 items');
    ...
    it('should fallback to WebLLM lastResult when streaming summary is empty', async () => {
      (mockBrainEngine.llm as { chat: ReturnType<typeof vi.fn> }).chat = vi
        .fn()
        .mockResolvedValue({ content: '' });
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue({ ok: false, error: 'WebLLM tool error' });
      ...
      expect(
        mockBrainEngine.onStreamEnd as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('WebLLM tool error');

      // Test with plain string result in WebLLM mode
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue('WebLLM plain string result');
      await executeToolCallsLoop(
        mockBrainEngine as unknown as BrainEngine,
        toolCallResponse,
        [],
      ...
      expect(
        mockBrainEngine.onStreamEnd as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('WebLLM plain string result');

      // Test with message property in WebLLM mode
      mockBrainEngine.executeTool = vi
        .fn()
        .mockResolvedValue({ message: 'WebLLM message result' });
      await executeToolCallsLoop(
        mockBrainEngine as unknown as BrainEngine,
        toolCallResponse,
        [],
      ...
      expect(
        mockBrainEngine.onStreamEnd as ReturnType<typeof vi.fn>
      ).toHaveBeenCalledWith('WebLLM message result');
    ...
    it('should handle malformed JSON in tool call arguments and throwing onToolNotFound hook', async () => {
      ...
      const toolCallResponse = {
        toolCalls: [
          {
            id: 'call_bad',
            function: { name: 'unknown_tool', arguments: '{invalid JSON' }
          }
        ],
        message: { content: '' }
      };
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('onToolNotFound 回呼執行錯誤'),
        expect.any(Error)
      );
