 ...
 */
export const DEFAULT_STT_MESSAGES: Record<string, Record<string, string>> = {
  'zh-TW': {
 ...
 * @param key - Message dictionary key identifier.
...
/**
 * Extended internal state for the Speech-to-Text (STT) engine.
 */
export interface InternalSTTEngineState extends STTEngineState {
...
export function initDefaultSTTEngine(options: STTEngineOptions = {}): STTEngine {
  ...
  const store = createBaseStore<InternalSTTEngineState>({
    micStream: null,
    micAudioCtx: null,
    micAnalyser: null,
    micData: null,
  ...
  const ensureMicMonitor = async (): Promise<void> => {
  ...
  const engine: STTEngine = {
    ...
    async startListening(): Promise<void> {
      ...
      recognitionInstance.onstart = (): void => {
      ...
      recognitionInstance.onend = (): void => {
      ...
      recognitionInstance.onaudiostart = (): void => {};
      recognitionInstance.onspeechstart = (): void => {
        state.noSpeechRuns = 0;
      };
      recognitionInstance.onspeechend = (): void => {};
      recognitionInstance.onaudioend = (): void => {};
